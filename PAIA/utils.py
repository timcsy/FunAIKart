...
def open_env() -> Tuple[UnityEnvironment, str]:
    ...
    # Behavior infomation
    debug_print('Behaviors:\n')
    debug_print(dict(env.behavior_specs))
    debug_print("--------------------------------")

    # We will only consider the first Behavior
    behavior_name = list(env.behavior_specs)[0]
    debug_print('Name of the behavior: ' + str(behavior_name))
    debug_print('--------------------------------')
...
def online(env: UnityEnvironment, behavior_name: str, random: bool = False) -> None:
    ...
    for episode in range(1, config.MAX_EPISODES + 1):
        ...
        while not done:
            ...
            debug_print('Episode: ' + str(episode) + ', Step: ' + str(step_num))
            ...
            for agent_id in terminal_steps:
                ...
                # Show the summary
                debug_print()
                debug_print('Done, Behavior: ' + str(behavior_name) + ', Agent: ' + str(agent_id) + ', Episode: ' + str(episode) + ', Total Steps: ' + str(step_num))
        ...
        debug_print('Finish, Behavior: ' + str(behavior_name) + ', Agent: ' + str(agent_id) + ', Episodes: ' + str(episode) + ', Total Steps: ' + str(total_steps))
...
def load_from_demo(filename: Optional[str] = None) -> None:
    ...
    for index in range(buffer.num_experiences):
        ...
        # Show the reward
        reward = get_rewards_from_buffer(buffer, index)
        ...
        if buffer[BufferKey.DONE][index]:
            debug_print()
            debug_print('Done, Demo: ' + str(filename) + ', Episode: ' + str(episode) + ', Total Steps: ' + str(step_num))
            episode += 1
            step_num = 0
        ...
        if index == buffer.num_experiences - 1:
            debug_print()
            debug_print('Finish, Demo: ' + str(filename) + ', Episodes: ' + str(episode) + ', Total Steps: ' + str(index))
...
def heuristic(env: UnityEnvironment, behavior_name: str, filename: Optional[str] = None) -> None:
    ...
    while not done:
        ...
        # Show the step information
        debug_print('Step: ' + str(total_steps))
        ...
        debug_print()
        debug_print('Finish, Behavior: ' + str(behavior_name) + ', Agent: ' + str(agent_id) + ', Total Steps: ' + str(total_steps))
